...
export default function CareerFitList() {
  ...
  const handleDelete = async (id) => {
    if (!await window.customConfirm('Are you sure you want to delete this career fit goal?')) return
    try {
      const token = localStorage.getItem('token')
      const response = await axios.delete(`${BASE_URL}/myadmin/career-fit/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.data?.status) {
        fetchData()
  ...
  const handleToggleStatus = async (id) => {
    try {
      const token = localStorage.getItem('token')
      const response = await axios.patch(`${BASE_URL}/myadmin/career-fit/status/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.data?.status) {
        fetchData()
      ...
      <div className="bg-[#f6f6ff] rounded-2xl shadow-md hover:shadow-[0_8px_30px_rgba(99,102,241,0.15)] transition-shadow border border-slate-100 transition-colors overflow-hidden flex flex-col flex-1 min-h-0">
        <div className="bg-[#144f36] rounded-t-2xl p-5 flex justify-between items-center shadow-md relative overflow-hidden group">
          ...
          <div className="flex items-center relative z-10">
            ...
            <h2 className="text-white font-bold tracking-wide text-2xl drop-shadow-[0_2px_4px_rgba(0,0,0,0.2)]">Career Fit</h2>
          </div>

          <button
            onClick={() => navigate('/career-fit/add')}
                ...
                {loading ? (
                  ...
                  data.map((row, index) => (
                    <tr key={row._id} className="border-b border-slate-200 dark:border-gray-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top">
                        {row.m_cf_icon ? (
                          <img src={getImageUrl(row.m_cf_icon)} alt={row.m_cf_title} className="w-10 h-10 object-cover rounded" />
                        ) : (
                          <div className="w-10 h-10 bg-slate-100 border border-slate-200 rounded flex items-center justify-center text-[10px] text-slate-400">No Icon</div>
                        )}
                      </td>
                      <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top">
                      ...
                      <td className="px-4 py-4 border-r border-slate-200 dark:border-gray-800/50 align-top text-center">
                        <button
                          ...
                          className={`text-white px-3 py-1.5 rounded-full text-xs transition-colors ${row.m_cf_status === 1 ? 'bg-[#144f36]' : 'bg-red-500'}`}
                        >
                          {row.m_cf_status === 1 ? 'Active' : 'Inactive'}
                        </button>
                      </td>
                      <td className="px-4 py-4 align-top">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => navigate(`/career-fit/edit/${row._id}`, { state: { careerFit: row } })}
